...
6. 在 `<feature>_feature.dart` 定义 `XxxFeature extends AppFeature`，声明唯一 key、显式 priority、允许环境等 `AppFeatureMetadata`，并暴露 routes、tabs 与 providerOverrides。
...
- Feature key 是否唯一、priority 是否显式；route path 与 Tab key 是否会冲突，Tab route 是否由所属 Feature 声明。
